...
import { View, Text, TextInput, TouchableOpacity, ScrollView, Image, Alert, StyleSheet, ImageBackground } from 'react-native';
import { Ionicons } from '@expo/vector-icons'; // Expo icon library
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, onAuthStateChanged } from 'firebase/auth';
import app from '../components/firebaseConfig'; // Ensure Firebase config is correctly imported
...
export default function SignInUpScreen({ navigation }) {
  ...
  // Effect to track user authentication state
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      if (currentUser) {
        setUser(currentUser); // Update user state
        navigation.navigate('MainApp'); // Navigate to Home
      } else {
          ...
          <View style={styles.authContainer}>
            ...
            <View style={styles.passwordContainer}>
              ...
              <TouchableOpacity onPress={() => setIsPasswordVisible(!isPasswordVisible)} style={styles.eyeIcon}>
                <Ionicons name={isPasswordVisible ? 'eye' : 'eye-off'} size={24} color="#777" />
              </TouchableOpacity>
            </View>
